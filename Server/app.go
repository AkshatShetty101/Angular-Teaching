...
func main() {
	...
	router.HandleFunc("/", listAllBooks).Methods("GET")
	router.HandleFunc("/add", insertBooks).Methods("PUT")
	router.HandleFunc("/flaky", flakyListAllBooks).Methods("GET")
...
func flakyListAllBooks(w http.ResponseWriter, r *http.Request) {
	fmt.Println("\nAt /flaky[GET] endpoint.")
	defer r.Body.Close()
	if rand.Float64() > 0.5 {
		// Querying DB
		var books []Book
		...
		http.Error(w, "Flaked Out", http.StatusInternalServerError)
	}
}
